...
class Symmetric_DDPG_Agent(object):
    ...
    def update_policy(self, epoch, sample):
...
class Copy_DDPG_Agent(object):
    def __init__(self, env, config, reference_pred, index):
        self.env = env
        self.index = index
        self.learning_agent = True
...
class DDPG_Runner():
                ...
                if all(done_n) or step == self.n_steps:
                    ...
                    for j in range(self.num_agents):
                        if info_n[j]['active'] and self.agents[j].learning_agent:
                            ...
                            self.replay_buffer.push(obs_n[j], act_n[j], reward_n[j], next_obs_n[j], done_n[j])
                    ...
                    for j, ag in enumerate(self.agents):
                        if ag.learning_agent:
                            if info_n[j]['active']:
                                self.replay_buffer.push(obs_n[j], act_n[j], reward_n[j], next_obs_n[j], done_n[j])

                            if len(self.replay_buffer) > self.batch_size and train_steps % self.update_steps == 0:
                                if ag.copy_agent:
                                    # update using params of symmetric agent
                                    ag.update_policy(epoch)
                                else:
                                    # normal update
                                    sample = self.replay_buffer.sample(self.batch_size)
                                    ag.update_policy(epoch, sample)
    ...
    def test(self, render=True):
        ...
        plot_multiagent_performance(self.checkpoint_path, steps, mean_steps, median_steps,
